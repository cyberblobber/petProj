...
import FilePicker  from '@petProj/react-native'
import  usepetProj  from '@petProj/react/lib/usepetProj'
...
export default function App () {
  ...
  const petProj = usepetProj(() => {
    return new petProj({ autoProceed: true, debug: true })
      .use(Tus, {
  ...
  useEffect(() => {
    petProj.on('upload-progress', (file, progress) => {
      setState({
        progress: progress.bytesUploaded,
        total: progress.bytesTotal,
        totalProgress: petProj.state.totalProgress,
        uploadStarted: true,
      })
    })
    petProj.on('upload-success', () => {
      // console.log(file.name, response)
    })
    petProj.on('complete', (result) => {
      setState({
    ...
    petProj.on('info-visible', () => {
      const { info } = petProj.getState()
      setState({
        info,
      })
      console.log('petProj-info:', info)
    })
    petProj.on('info-hidden', () => {
      setState({
        info: null,
      })
    })
  ...
  return (
    ...
    >
      <Text
        style={styles.title}
      >
        petProj in React Native
      </Text>
      <View style={{ alignItems: 'center' }}>
        <Image
          style={styles.logo}
          // eslint-disable-next-line global-require
          source={require('./assets/petProj-logo.png')}
        />
      </View>
      ...
      {petProj && <FileList petProj={petProj} />}
