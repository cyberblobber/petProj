...
import DropTarget from '@petProj/drop-target'
import Dashboard from '@petProj/dashboard'
import Transloadit from '@petProj/transloadit'
...
class MarkdownTextarea {
  ...
  setuppetProj = () => {
    this.petProj = new petProj({ autoProceed: true })
      .use(Transloadit, {
    ...
    this.petProj.on('complete', (result) => {
  ...
  uploadFiles = (files) => {
    const filesForpetProj = files.map(file => {
      return {
        data: file,
        type: file.type,
        name: file.name,
    ...
    this.petProj.addFiles(filesForpetProj)
  }
}
