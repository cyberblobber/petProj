# petProj + AWS S3 Example

This example uses @petProj/companion with a custom AWS S3 configuration.
...
`@petProj/companion` in a `.env` file. You may find useful to first copy the
`.env.example` file:
...
corepack yarn workspace @petProj-example/aws-companion start
```
