...
import ReduxStore from '@petProj/store-redux'
import * as petProjReduxStore from '@petProj/store-redux'
...
const reducer = combineReducers({
  counter,
  // You don't have to use the `petProj` key. But if you don't,
  // you need to provide a custom `selector` to the `petProjReduxStore` call below.
  petProj: petProjReduxStore.reducer,
})

let enhancer = applyMiddleware(
  petProjReduxStore.middleware(),
  logger,
)
...
const store = configureStore({
  ...
  middleware: (getDefaultMiddleware) => getDefaultMiddleware({
    serializableCheck: {
      ignoredActions: [petProjReduxStore.STATE_UPDATE],
      ignoreState: true,
    },
  }),
})
...
// petProj using the same store
const petProj = new petProj({
  id: 'redux',
  store: new ReduxStore({ store }),
  // If we had placed our `reducer` elsewhere in Redux, eg. under an `petProj` key in the state for a profile page,
  // we'd do something like:
  //
  // store: new ReduxStore({
  //   store: store,
  //   id: 'avatar',
  //   selector: state => state.pages.profile.petProj
  // }),
  debug: true,
})
...
petProj.use(Tus, { endpoint: 'https://tusd.tusdemo.net/files/' })
